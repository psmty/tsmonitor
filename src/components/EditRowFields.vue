<template>
  <div class="space-y-4 h-full overflow-y-auto px-4">
    ...
    <div>
      <label for="environment" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Environment</label>
      <select id="environment" v-model="editData.environment"
    ...
    <div>
      <label for="has-integration" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Has
        integration</label>
      <select id="has-integration" v-model="editData.hasIntegration"
        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
    ...
    <div>
      ...
      <select id="resource" v-model="editData.resource"
        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
        <option v-for="item in props.resources" :key="item" :value="item">{{ item }}</option>
      </select>
    </div>
