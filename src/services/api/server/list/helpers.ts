...
const uuidSeparator = '$%';
const getUUIDURL = (url: string) => {
  return `${url}${uuidSeparator}${crypto.randomUUID()}`
}

export const getRealURL = (uuidUrl: string) => {
  return uuidUrl.split(uuidSeparator)[0];
}
